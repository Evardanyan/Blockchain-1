...
class MultifunctionalMapper {
     ...
     */
    public static final Function<List<IntUnaryOperator>, UnaryOperator<List<Integer>>> MULTIFUNCTIONAL_MAPPER = lstFunc->
     ...
     */
    public static final UnaryOperator<List<Integer>> MULT_TWO_AND_THEN_ADD_ONE_TRANSFORMATION = x -> x.stream().map(y -> y * 2 + 1).collect(Collectors.toList());
     ...
     * The operator returns transformed integer list.
     */
    public static final UnaryOperator<List<Integer>> SQUARE_AND_THEN_GET_NEXT_EVEN_NUMBER_TRANSFORMATION = x -> x.stream().map(y -> y * y).map(z -> z % 2 == 0 ? z + 2 : z + 1).collect(Collectors.toList());
