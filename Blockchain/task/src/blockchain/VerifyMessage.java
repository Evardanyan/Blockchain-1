...
public class VerifyMessage {
    ...
    public static void VerifyMessage(List<byte[]> text, String keyFile) throws Exception {
//        ObjectInputStream in = new ObjectInputStream(new FileInputStream(filename));
//        this.list = (List<byte[]>) in.readObject();
//        in.close();

        System.out.println(verifySignature(text.get(0), text.get(1), keyFile) ? "VERIFIED MESSAGE" +
                "\n----------------\n" + new String(list.get(0)) : "Could not verify the signature.");
    }

    //Method for signature verification that initializes with the Public Key,
    //updates the data to be verified and then verifies them using the signature
    public static boolean verifySignature(byte[] data, byte[] signature, String keyFile) throws Exception {
